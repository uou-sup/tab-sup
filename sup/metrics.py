...
class TabMetrics(object):
    ...
    def evaluate_density(self, syn_data):
        ...
        metadata['columns'] = {int(key): value for key, value in
                               metadata['columns'].items()}  # ensure that keys are all integers?
    ...
    def evaluate_mle(self, syn_data):
        ...
        mle_score = overall_scores['best_rmse_scores']['XGBRegressor']['RMSE'] if task_type == 'regression' else \
        overall_scores['best_auroc_scores']['XGBClassifier']['roc_auc']
        out_metrics = {
            "mle": mle_score,
        }
        out_extras = {
    ...
    def evaluate_dcr(self, syn_data):
        ...
        dcrs_real = []
        dcrs_test = []
        batch_size = 10000 // cat_real_data_oh.shape[
            1]  # This esitmation should make sure that dcr_real and dcr_test can be fit into 10GB GPU memory
